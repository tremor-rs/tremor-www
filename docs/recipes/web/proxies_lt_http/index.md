...
Tremor based HTTP proxy application.

## Setup

:::tip
All the code here is available in the [git repository](https://github.com/tremor-rs/tremor-www/tree/main/docs/recipes/proxies_lt_http) as well.
:::

### Sources and sinks

We configure a rest onramp listening on port 9139, that is meant to be a proxy for our [example HTTP server](../servers_lt_http) (configured as en endpoint in the rest offramp here).
...
All the [testing examples](../servers_lt_http#testing) for the example HTTP server should work from here as well, with the port `8139` there swapped to our proxy application port `9139`.
