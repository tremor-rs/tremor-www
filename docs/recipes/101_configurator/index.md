...
An application built using tremor using the [qos::wal](/docs/queries/operators#qoswal) operator introduced in 0.9 and the `$correlation` metadata feature introduced in 0.11, allowing for centralized configuration across services and their component nodes.

The main task of the Configurator is to distribute config changes to a group of upstream tremor nodes running the [_Quota Service_](/docs/recipes/quota_service).
...
* [Quota Service](/docs/recipes/quota_service) Node 1: [http://localhost:8139](http://localhost:8139)
* [Quota Service](/docs/recipes/quota_service) Node 2: [http://localhost:8140](http://localhost:8140)
