...
This example is intentionally light on the processing but you can imagine doing arbitrary processing based on the event data here (as well as dynamically changing the confiuration for the [websocket offramp](../../connectors/offramps#ws) via its metadata variables --  eg: things like the server url).
