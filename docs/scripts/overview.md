...
:::tip
A _tuple pattern_ matches a _target_ value if the *target* is an array and **each** test matches the positionally correspondent value in the *target*. The *target* needs to be **at least as long** as the _pattern_ but **can be longer** if the *pattern* ends with `...`.
...
:::tip
An _array pattern_ matches a target value if the _target_ is an array and **each** test in the pattern matches **at least for one** element in the *target* indiscriminate of their positions.
