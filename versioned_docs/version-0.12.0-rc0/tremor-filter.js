...
const prism_js = require('prismjs');
...
var errFile = path.join(folder,  "tremor-filter.err");
var errorLog = fs.createWriteStream(errFile);

function tremor(type, value, format, meta) {
    if (type != "CodeBlock") return null;
    var attrs = value[0], id = attrs[0], classes = attrs[1];
    if (classes.indexOf('mermaid') >= 0) {
	console.err('got here mermaid');
        return mermaid(type, value, format, meta)
    } else if (classes.indexOf('tremor') >= 0) {
	console.err('got here tremor');
        prism('tremor', type, value, format, meta)
    } else if (classes.indexOf('trickle') >= 0) {
	console.err('got here trickle');
        prism('trickle', type, value, format, meta)
    } else if (classes.indexOf('troy') >= 0) {
	console.err('got here troy');
        prism('troy', type, value, format, meta)
    } else if (classes.indexOf('ebnf') >= 0) {
	console.err('got here ebnf');
        prism('troy', type, value, format, meta)
    }
    return null;
}

function prism(kind, type, value, format, meta) {
    var content = value[1];
    console.err(kind);
    return pandoc.Para(
        [
...
function mermaid(type, value, format, meta) {
    ...
    var outdir = path.dirname(tmpfileObj.name);
    console.log(outdir);
...
pandoc.toJSONFilter(function(type, value, format, meta) {
    ...
    return tremor(type, value, format, meta);
});
