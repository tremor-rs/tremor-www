...
When appending to an array we can use the [`array::push`](../reference/library/std/array#pusharray-element) function
...
To drop a percentage of all events, functions in the [random](../reference/library/std/random) module can be used.
...
Note that this is different from the case where a variable is set to `null`, for which we can do [function-based](../reference/library/std/type#is_nullvalue) checks as well as pattern-match with [match](overview#match).
