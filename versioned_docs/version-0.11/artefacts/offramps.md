...
It is recommended to batch events sent to this sink using the [generic::batch operator](/docs/0.11/tremor-query/operators#genericbatch) to reduce the overhead
...
The default [codec](/docs/0.11/artefacts/codecs#json) is `json`.
...
When used as [Linked Transport](/docs/0.11/operations/linked-transports) the same handling is applied to the incoming HTTP response, giving precedence to the `Content-Type` header and only falling back to the configured `codec`.
...
The default codec is [json](/docs/0.11/artefacts/codecs#json).
...
Set metadata variables (for reply with [linked transports](/docs/0.11//operations/linked-transports)):
